...
function ActionOnQuery(ArrayCmd, Query,ProcessAction,IdAction,TagRepl,ArrayStream )
{
  ...
  ### IntCounterID used to increment a counter to allow 
  ### index of ArrayCmd own another pair than only ( IdAction, "stream-return" )
  ### Because it will be overwrited, adding inside ArrayCmd with following
  ### new pair : ( IdAction, "stream-return", IntCounterID ) will add 
  ### every output generated by ExpandAppsCall. Adding any other information
  ### also starting with pair ( IdAction, "stream-return", IntCounterID ) , like
  ### Query to specify inside the Array, this "stream-return" is based on query
  ### Query, can leave the result from ActionOnQuery being more explicit. 
  IntCounterID=0 ; 
  for( idxAS in ArraySplit )
	 {
	   ...
	   if( tolower( ProcessAction ) == "true" )
	   {
				 ...
				 if( IdAction != "" )
				 {
					 ...
					 if( (idxItemAction,"redir") in  ArrayCmd )
      {
       ArrayIDCmd["1"]="#/usr/bin/uuidgen -t" ; 
       ArrayIDCmd["1", "redir"]="/dev/stderr" ; 
       ArrayIDCmd["1", "stream-return"]="" ; 
       ArrayCmd[IdAction, "stream-return"]=ExpandAppsCall( ArrayIDCmd, 1, DevStdOut ) ; 
       StrUUID=ArrayCmd[IdAction, "stream-return"] ; 
       ### ArrayCmd[IdAction, "stream-return" ] ---> will hold 
       ### the ID for external recovery of the information. 
       ### 
       ### So It will hold the UUID-ID for this execution and will be
       ### available over [THIS-UUID-NAME] if it had to be stored
       ### somewhere.
       ArrayCmd[IdAction, "stream-return", IntCounterID ]=StrUUID ;  
       ArrayCmd[IdAction, StrUUID]=ExpandAppsCall( ArrayCmd, IdAction, DevStdOut ) ;  
      }
					 ArrayCmd[IdAction]=OldApps ; 
					}	
     printf("ProcessAction=True: Match:%s\nIssued UUID:%s for this action.\n",ArraySplit[idxAS],ArrayCmd[IdAction, "stream-return"]) > localStdErr  ; 	
				}
